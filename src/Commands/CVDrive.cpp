...
{
	if(NetworkTablesInterface::UFound()){
		distance = NetworkTablesInterface::GetDistance();
		angleAz = NetworkTablesInterface::GetAzimuth();
		angleAl = NetworkTablesInterface::GetAltitude();
	}
	else{
		distance = 0;
		angleAz = 0;
	...
	anglePid = new NewPIDController(.05, 0, 0, angleAz, false);
}
